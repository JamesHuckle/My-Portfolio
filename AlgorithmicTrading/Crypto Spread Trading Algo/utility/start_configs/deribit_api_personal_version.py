...
class RestClient(object):
    ...
    def request(self, action, data):
        ...
        if action.startswith("/api/v1/private/"):
            ...
            response = self.session.post(self.url + action, data=data, headers={'x-deribit-sig': signature}, verify=True,timeout=(0.1, 0.3))
        else:
            response = self.session.get(self.url + action, params=data, verify=True,timeout=(0.1, 0.3))
    ...
    def buy(self, instrument, quantity, price, type, postOnly=None, label=None):
        options = {
            "instrument": instrument,
            "quantity": quantity,
            "type":type,
        ...
        if type == "stop_market":
            options["stopPx"] = price
            options["execInst"] = "mark_price"
        else:
            options["price"] = price
    ...
    def sell(self, instrument, quantity, price, type, postOnly=None, label=None):
        options = {
            "instrument": instrument,
            "quantity": quantity,
            "type":type,
    ...
    def edit(self, orderId, quantity, price, type="limit"):
        options = {
            "orderId": orderId,
            "quantity": quantity,
        }
    ...
    def orderstate(self, orderId):
        return self.request("/api/v1/private/orderstate", {'orderId': orderId})
